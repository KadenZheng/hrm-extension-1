!(function (global, factory) {
    "object" == typeof exports && "undefined" != typeof module
        ...
        : factory(((global = "undefined" != typeof globalThis ? globalThis : global || self).loadPyodide = {}));
                ...
                (SAFARI_NATIVE_CODE_REGEXP = /^(eval@)?(\[native code])?$/),
                {
                    ...
                    parseFFOrSafari: function (error) {
                        return error.stack
                            ...
                            .map(function (line) {
                                if (
                                    (line.indexOf(" > eval") > -1 && (line = line.replace(/ line (\d+)(?: > eval line \d+)* > eval:\d+:\d+/g, ":$1")),
                    ...
                    parseOpera10: function (e) {
                        for (
                            var lineRE = /Line (\d+).*script (?:in )?(\S+)(?:: In function (\S+))?$/i, lines = e.stacktrace.split("\n"), result = [], i = 0, len = lines.length;
                            i < len;
                            i += 2
                        ) {
                            var match = lineRE.exec(lines[i]);
    ...
    async function loadPyodide(options = {}) {
        ...
        const default_config = {
            ...
            Module = (function () {
                let Module = {
                    ...
                    preRun: [],
                    quit: (status, toThrow) => {
                        throw ((Module.exited = { status: status, toThrow: toThrow }), toThrow);
                    },
                };
                return Module;
            })();
        (Module.print = config.stdout),
            ...
            Module.preRun.push(() => {
                for (const mount of config._node_mounts) Module.FS.mkdirTree(mount), Module.FS.mount(Module.NODEFS, { root: mount }, mount);
            }),
                    ...
                    nativeFSAsync = {
                        ...
                        getLocalSet: (mount) => {
                            ...
                            for (; check.length; ) {
                                let path = check.pop(),
                                    stat = FS.stat(path);
                                FS.isDir(stat.mode) && check.push.apply(check, FS.readdir(path).filter(isRealDir).map(toAbsolute(path))),
                        ...
                        loadLocalEntry: (path) => {
                            const node = FS.lookupPath(path).node,
                                stat = FS.stat(path);
                            if (FS.isDir(stat.mode)) return { timestamp: stat.mtime, mode: stat.mode };
                        ...
                        loadRemoteEntry: async (handle) => {
                            ...
                            if ("directory" === handle.kind) return { mode: nativeFSAsync.DIR_MODE, timestamp: new Date() };
                        ...
                        storeRemoteEntry: async (handles, path, entry) => {
                            const parentDirHandle = handles.get(PATH.dirname(path)),
                                handle = FS.isFile(entry.mode)
                                    ? await parentDirHandle.getFileHandle(PATH.basename(path), { create: !0 })
                                    : await parentDirHandle.getDirectoryHandle(PATH.basename(path), { create: !0 });
                            if ("file" === handle.kind) {
        ...
        if (
            (pyodide.version.includes("dev") || API.setCdnUrl(`https://cdn.jsdelivr.net/pyodide/v${pyodide.version}/full/`),
            await API.packageIndexReady,
            API._pyodide._importhook.register_module_not_found_hook(API._import_name_to_package_name),
            "0.22.1" !== API.repodata_info.version)
        )
            throw new Error("Lock file version doesn't match Pyodide version");
        return (
            API.package_loader.init_loaded_packages(),
            config.fullStdLib && (await pyodide.loadPackage(API._pyodide._importhook.UNVENDORED_STDLIBS)),
            API.initializeStreams(config.stdin, config.stdout, config.stderr),
            pyodide
        );
    }
    (globalThis.loadPyodide = loadPyodide), (exports.loadPyodide = loadPyodide), (exports.version = "0.22.1"), Object.defineProperty(exports, "__esModule", { value: !0 });
});
